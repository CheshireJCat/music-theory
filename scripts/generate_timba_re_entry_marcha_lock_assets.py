...
ASSET_DIR = ROOT / "assets" / "2026-07-03-timba-re-entry-marcha-lock"
...
def save_piano_chart():
    img = Image.new("RGB", (1560, 1040), "#F5EBDD")
    draw = ImageDraw.Draw(img)
    draw.text((54, 38), "钢琴图：Timba Re-Entry Marcha Lock", fill="#2E2419", font=font(52, bold=True))
    draw.text(
        (54, 102),
        "Re-Entry Hit 解决的是“怎么撞回来”，Marcha Lock 解决的是“撞回来以后怎样立刻把推进重新锁稳”。重点是回击后马上回到持续推进型钢琴句法。",
        fill="#66584B",
        font=font(28),
    )

    draw.rounded_rectangle((74, 176, 1486, 354), 26, fill="#FFF8EE", outline="#DCC8A8", width=3)
    draw.text((102, 212), "示例调性：Gm。D pedal 悬挂 -> Gm9 / C13 回击 -> 右手立刻回到 Gm9 anticipations，左手继续把和声入口锁紧。", fill="#8B5A2A", font=font(24, bold=True))
    draw.text((102, 262), "判断标准：如果重击后 groove 继续滚动，而不是停成两下口号，才算真的 lock 住 marcha。", fill="#58616B", font=font(24))

    labels = ["1", "&", "2", "&", "3", "&", "4", "&"]
    hit_zone = [0, 2]
    lock_left = [3, 4, 7]
    lock_right = [1, 3, 5, 6, 7]
    start_x = 146
    y_top = 484
    y_mid = 650
    y_bottom = 816
    draw.text((96, 436), "re-entry hit", fill="#A4562B", font=font(30, bold=True))
    draw.text((96, 602), "左手 lock 入口", fill="#4A78AE", font=font(30, bold=True))
    draw.text((96, 768), "右手 marcha 推进", fill="#2E7D59", font=font(30, bold=True))
    for idx, label in enumerate(labels):
        x = start_x + idx * 160
        fill_top = "#C97847" if idx in hit_zone else "#EEE8DF"
        fill_mid = "#5D88C5" if idx in lock_left else "#E7EDF3"
        fill_bottom = "#3E9A72" if idx in lock_right else "#E6F2EB"
    ...
    cards = [
        (92, 900, "#FFF2DD", "#D9B57F", "回击后别停住", "Gm9 与 C13 打完后，右手必须立刻回到 anticipations，段落才会重新滚起来。"),
        (560, 900, "#EEF3FF", "#AFC2E2", "左手继续锁和声", "左手不是跟着口号结束，而是继续把主循环入口钉在正确拍点上。"),
        (1028, 900, "#EEF8F0", "#B8D5C0", "推进感重于音量", "Marcha lock 的核心不是更大声，而是让持续前冲重新变稳定。"),
    ]
    ...
    img.save(ASSET_DIR / "piano-timba-re-entry-marcha-lock.png")


def save_guitar_chart():
    img = Image.new("RGB", (1560, 1080), "#EEF4F2")
    draw = ImageDraw.Draw(img)
    draw.text((56, 38), "吉他图：Re-Entry 之后的 Marcha Lock", fill="#17313A", font=font(52, bold=True))
    draw.text(
        (56, 102),
        "吉他这里不只是跟着 hit 一起砸两下，而是要在回击之后马上恢复高位短促 comping，帮整队把 marcha 重新锁成持续推进。",
        fill="#4C636C",
        font=font(26),
    )

    draw.rounded_rectangle((88, 164, 1470, 332), 26, fill="#FFFDF9", outline="#C7D1D5", width=3)
    draw.text((116, 202), "练习循环：| Gm9 hit . C13 hit . | Gm9 . . C13 . . |。前半是回击，后半是把主 groove 重新卡稳。", fill="#223943", font=font(24, bold=True))
    draw.text((116, 258), "手感标准：两下 hit 要像口号，后面的 comping 要像齿轮重新咬合。", fill="#516771", font=font(23))

    positions = [88, 402, 716, 1030]
    grids = [
        ("Gm9 hit", "3 x 3 3 3 5", [(1, 0, "1"), (1, 2, "b7"), (1, 3, "9"), (1, 4, "b3"), (3, 5, "5")], {1: "X"}, "第一下把门撞开。", "#2F8B61"),
        ("C13 hit", "8 x 8 9 10 10", [(1, 0, "1"), (1, 2, "b7"), (2, 3, "3"), (3, 4, "13"), (3, 5, "5")], {1: "X"}, "第二下把回归说清。", "#C97B3F"),
        ("Gm9 lock", "x 10 12 10 11 x", [(1, 1, "1"), (3, 2, "5"), (1, 3, "b7"), (2, 4, "9")], {0: "X", 5: "X"}, "马上恢复高位短促推进。", "#5C7DB8"),
        ("C13 lock", "x 3 2 3 5 x", [(1, 1, "1"), (1, 2, "3"), (1, 3, "b7"), (3, 4, "13")], {0: "X", 5: "X"}, "把下一轮 groove 接牢。", "#A15B6B"),
    ]
    ...
    draw.text(
        (112, 958),
        "右手口令：砸 - 停 - 砸 - 收 | 点 - 留 - 点 - 追。重点击不是更满，而是 hit 之后马上重新进入推进型呼吸。",
        fill="#4D5F68",
        font=font(23, bold=True),
    )

    img.save(ASSET_DIR / "guitar-timba-re-entry-marcha-lock.png")


def save_structure_chart():
    img = Image.new("RGB", (1500, 1040), "#F8F5EF")
    draw = ImageDraw.Draw(img)
    draw.text((56, 40), "结构图：Timba Re-Entry Marcha Lock", fill="#2D261D", font=font(50, bold=True))
    draw.text(
        (56, 104),
        "这一课只推进一步：昨天学的是“怎样回击”，今天学的是“回击之后怎样立刻把 marcha 锁稳，不让段落重新散掉”。",
        fill="#655A4E",
        font=font(27),
    )
    ...
    rows = [
        ("统一回击", ["Gm9", "-", "C13", "-", "-", "-", "-", "-"], [0, 2], "前两下先把入口砸清楚。"),
        ("左手 lock", ["-", "-", "-", "Gm", "C13", "-", "-", "D7"], [3, 4, 7], "左手在回击后立刻恢复和声入口。"),
        ("右手 marcha", ["-", "提", "-", "推", "-", "咬", "追", "冲"], [1, 3, 5, 6, 7], "右手用 anticipations 把推进重新锁稳。"),
    ]
    ...
    for row_idx, (label, cells, active, desc) in enumerate(rows):
        ...
        for i, cell in enumerate(cells):
            ...
            state = "lock" if i in active else "rest"
    ...
    draw.text((118, 908), "判断标准：听感应该是“撞回来以后 groove 立刻重新滚动”，而不是只剩两下重音然后又空掉。", fill="#5F5547", font=font(23, bold=True))

    img.save(ASSET_DIR / "timba-re-entry-marcha-lock-structure.png")
