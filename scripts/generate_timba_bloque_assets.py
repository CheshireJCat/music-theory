...
ASSET_DIR = ROOT / "assets" / "2026-06-25-timba-bloque"
...
def save_piano_chart():
    img = Image.new("RGB", (1560, 1040), "#F6EEE3")
    draw = ImageDraw.Draw(img)
    draw.text((54, 38), "钢琴图：Timba Bloque 的统一击打", fill="#2E2419", font=font(52, bold=True))
    draw.text(
        (54, 102),
        "Bloque 是 Timba 里整队突然一起切出的重击或断点。钢琴不是一直滚动，而是在 marcha 流动中故意留下一个统一爆点，让所有人同时“砸”在同一处。",
        fill="#66584B",
        font=font(28),
    )

    draw.rounded_rectangle((74, 176, 1486, 354), 26, fill="#FFF9F1", outline="#DCC8A8", width=3)
    draw.text((102, 212), "示例调性：Gm。前一拍保持 marcha，进入 bloque 时全队在同一落点短促重击。", fill="#8B5A2A", font=font(28, bold=True))
    draw.text((102, 262), "理解方式：marcha 负责持续推进，bloque 负责在推进中制造“突然集体转身”的爆发点。", fill="#58616B", font=font(24))
    ...
    draw.text((96, 602), "bloque 击打点", fill="#9C5F2A", font=font(30, bold=True))
    for idx, label in enumerate(labels):
        x = start_x + idx * 160
        fill_top = "#5D88C5" if idx in marcha else "#E7EDF3"
        fill_bottom = "#C98647" if idx in bloque else "#E8E3D9"
    ...
    cards = [
        (92, 790, "#EEF3FF", "#AFC2E2", "先铺推进", "先让 marcha 把能量积起来，bloque 才会像突然撞出来。"),
        (560, 790, "#FFF2DD", "#D9B57F", "再统一砸点", "bloque 的关键不是多，而是全队同一时间、同一口气。"),
        (1028, 790, "#EEF8F0", "#B8D5C0", "马上回 groove", "击打完不要拖，立刻回到 marcha 或下一层 groove。"),
    ]
    ...
    img.save(ASSET_DIR / "piano-timba-bloque.png")


def save_guitar_chart():
    img = Image.new("RGB", (1560, 1080), "#EEF4F2")
    draw = ImageDraw.Draw(img)
    draw.text((56, 38), "吉他图：配合 Bloque 的高位短刺点", fill="#17313A", font=font(52, bold=True))
    draw.text(
        (56, 102),
        "吉他在 bloque 里不能继续扫成长伴奏，而要和钢琴、贝斯、铜管一起切出统一攻击点。做法仍是高位和弦，但出声更短、更像刀口。",
        fill="#4C636C",
        font=font(26),
    )

    draw.rounded_rectangle((88, 164, 1470, 332), 26, fill="#FFFDF9", outline="#C7D1D5", width=3)
    draw.text((116, 202), "练习循环：前一小节做高位 comping，第二小节在 `& of 2` 和 `3` 做 bloque 重击，然后立刻停手。", fill="#223943", font=font(24, bold=True))
    draw.text((116, 258), "判断标准：你一出声，应该让乐句像“啪地一下转弯”，而不是继续铺底。", fill="#516771", font=font(23))

    positions = [88, 402, 716, 1030]
    grids = [
        ("Gm9", "3 x 3 3 3 5", [(1, 0, "1"), (1, 2, "b7"), (1, 3, "9"), (1, 4, "b3"), (3, 5, "5")], {1: "X"}, "marcha 段保持轻短。", "#2F8B61"),
        ("C13", "8 x 8 9 10 10", [(1, 0, "1"), (1, 2, "b7"), (2, 3, "3"), (3, 4, "13"), (3, 5, "5")], {1: "X"}, "用它把能量顶到 bloque 前。", "#C97B3F"),
        ("Ebmaj9", "x 6 5 7 6 x", [(1, 1, "1"), (1, 2, "3"), (2, 3, "7"), (1, 4, "9")], {0: "X", 5: "X"}, "bloque 后的颜色回落。", "#5C7DB8"),
        ("D7alt", "x 5 4 5 6 x", [(1, 1, "1"), (1, 2, "3"), (1, 3, "b7"), (2, 4, "b9")], {0: "X", 5: "X"}, "为下一轮再点火。", "#A15B6B"),
    ]
    ...
    draw.text(
        (112, 958),
        "右手口令：轻点 - 轻点 - 预备 - 砸 | 停 - 停 - 回 groove。Bloque 要像一个统一标点，不是持续扫弦。",
        fill="#4D5F68",
        font=font(24, bold=True),
    )

    img.save(ASSET_DIR / "guitar-timba-bloque.png")


def save_structure_chart():
    img = Image.new("RGB", (1500, 1040), "#F8F5EF")
    draw = ImageDraw.Draw(img)
    draw.text((56, 40), "结构图：Timba Bloque", fill="#2D261D", font=font(50, bold=True))
    draw.text(
        (56, 104),
        "Bloque 不是单独节奏型，而是整队在 groove 内部统一打出的“断点标点”。它常接在 marcha、bell 或 gear change 之后，让能量突然结块。",
        fill="#655A4E",
        font=font(27),
    )
    ...
    rows = [
        ("推进层", ["1", "&", "2", "&", "3", "&", "4", "&"], [1, 3, 5, 6, 7], "先由 marcha 或 comping 持续把下一拍拉近。"),
        ("bloque 层", ["1", "&", "2", "&", "3", "&", "4", "&"], [3, 4], "在 `& of 2` 到 `3` 形成统一重击。"),
        ("身体口令", ["留", "推", "留", "砸", "顶", "停", "等", "回"], [1, 3, 4, 7], "打完就收，下一拍再回到 groove。"),
    ]
    ...
    draw.text((118, 908), "判断标准：听感应该像“先一路往前推，再突然全队一起砸下标点”，而不是一直均匀铺开。", fill="#5F5547", font=font(24, bold=True))

    img.save(ASSET_DIR / "timba-bloque-structure.png")
